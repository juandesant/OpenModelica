...
def getVersion() {
  return sh (script: 'build/bin/omc --version | grep -o "v[0-9]\\+[.][0-9]\\+[.][0-9]\\+"', returnStdout: true)
}

void compliance(prefix='compliance') {
  standardSetup()
  unstash 'omc-clang'
  makeLibsAndCache('all')
  sh 'build/bin/omc -g=MetaModelica build/share/doc/omc/testmodels/ComplianceSuite.mos'
  sh "mv ${prefix}.html ${prefix}-current.html"
  sh "test -f ${prefix}.xml"
  // Only publish openmodelica-current.html if we are running master
  sh "cp -p ${prefix}-current.html ${prefix}${env.CACHE_BRANCH=='master' ? '' : ('-' + env.CACHE_BRANCH).replace('/','-')}-${getVersion()}.html"
  sh "test ! '${env.CACHE_BRANCH}' = 'master' || rm -f ${prefix}-current.html"
  stash name: "${prefix}", includes: "${prefix}-*.html"
  archiveArtifacts "${prefix}*${getVersion()}.html, ${prefix}.failures"
  // get rid of freaking %
  sh "sed -i.bak 's/%/\\&#37;/g' ${prefix}.ignore.xml && sed -i.bak 's/[^[:print:]]/ /g' ${prefix}.ignore.xml"
  junit "${prefix}.ignore.xml"
}
